...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
	...
	public class MultiplierTest
	{
		...
		public void ParallelEfficiencyTest()
		{
			...
			Matrix firstMatrix = new Matrix(size, size, true);
			Matrix secondMatrix = new Matrix(size, size, true);
			...
			Assert.IsTrue(parallelTime < time, $"Parallel time - {parallelTime}, Base time - {time}");
		}
